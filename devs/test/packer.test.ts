...
test("UInt/Int encode-decode and size", () => {
  ...
  const u_buf = packed_encode(u32, u_val);
  expect(u_buf.length).toBe(4);
  expect(packed_decode<number>(u32, u_buf)).toBe(u_val >>> 0);

  const i5: Packed = { $: "Int", size: 5 };
  const i_val = -3;
  const i_buf = packed_encode(i5, i_val);
  expect(i_buf.length).toBe(1); // 5 bits
  expect(packed_decode<number>(i5, i_buf)).toBe(i_val);
});
...
test("Struct bit packing size", () => {
  ...
  const buf = packed_encode(packed_t, val);
  expect(buf.length).toBe(6); // 42 bits
  expect(packed_decode<typeof val>(packed_t, buf)).toEqual(val);
});
...
test("List encoding size", () => {
  ...
  expect(packed_decode<number[]>(packed_t, buf)).toEqual(val);
});
...
test("Map encoding size", () => {
  ...
  const decoded = packed_decode<Map<number, number>>(packed_t, buf);
...
test("String encoding size", () => {
  ...
  const val_1 = "A";
  const buf_1 = packed_encode(packed_t, val_1);
  expect(buf_1.length).toBe(2); // 10 bits
  expect(packed_decode<string>(packed_t, buf_1)).toBe(val_1);

  const val_2 = "€"; // 3 bytes in UTF-8
  const buf_2 = packed_encode(packed_t, val_2);
  expect(buf_2.length).toBe(4); // 28 bits
  expect(packed_decode<string>(packed_t, buf_2)).toBe(val_2);
});
...
test("Nat encoding size", () => {
  ...
  expect(packed_decode<number>(packed_t, buf)).toBe(val);
});
...
test("Nested lists, tuples, maps, and strings (size + identity)", () => {
  ...
  const out = packed_decode<typeof val>(packed_t, buf);
...
test("BigInt paths for wide UInt/Int", () => {
  ...
  const u_buf = packed_encode(u70, u_val);
  expect(u_buf.length).toBe(9); // 70 bits
  expect(packed_decode<bigint>(u70, u_buf)).toBe(u_val);

  const i65: Packed = { $: "Int", size: 65 };
  const i_val = -(1n << 63n) + 42n;
  const i_buf = packed_encode(i65, i_val);
  expect(i_buf.length).toBe(9); // 65 bits
  expect(packed_decode<bigint>(i65, i_buf)).toBe(i_val);
});
...
test("Union tag ordering is alphabetical", () => {
  ...
  const buf_a = packed_encode(packed_t, { $: "a", value: 1 });
  const buf_z = packed_encode(packed_t, { $: "z", value: 1 });
...
test("GamePost union encode/decode (string fields)", () => {
  ...
  expect(packed_decode<typeof spawn>(game_post_t, packed_encode(game_post_t, spawn))).toEqual(spawn);
  expect(packed_decode<typeof down>(game_post_t, packed_encode(game_post_t, down))).toEqual(down);
  expect(packed_decode<typeof up>(game_post_t, packed_encode(game_post_t, up))).toEqual(up);
});
...
test("GamePost union sizes (string fields + u16 positions)", () => {
  ...
  const spawn = { $: "spawn", pid: 97, x: 200, y: 200 };
  expect(packed_encode(game_post_t, spawn).length).toBe(6);

  // down/up: 2 + 2 + 8 = 12 bits => 2 bytes.
  const down = { $: "down", pid: 97, key: { $: "w" } };
  const up = { $: "up", pid: 97, key: { $: "d" } };
  expect(packed_encode(game_post_t, down).length).toBe(2);
  expect(packed_encode(game_post_t, up).length).toBe(2);
});

test("decode throws on truncated buffer instead of reading zeros", () => {
  const u32: Packed = { $: "UInt", size: 32 };
  const buf = packed_encode(u32, 0xdeadbeef);
  expect(() => packed_decode(u32, buf.subarray(0, 2))).toThrow(RangeError);

  const s: Packed = { $: "String" };
  const s_buf = packed_encode(s, "hello");
  expect(() => packed_decode(s, s_buf.subarray(0, 2))).toThrow(RangeError);

  expect(() => packed_decode(u32, new Uint8Array(0))).toThrow(RangeError);
});
